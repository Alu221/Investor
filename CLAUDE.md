...
## ОБЯЗАТЕЛЬНАЯ ПРОВЕРКА АКТУАЛЬНЫХ ДАННЫХ (WebSearch)

**НИКОГДА не используй захардкоженные цифры из knowledge/ файлов для:**
- Ключевая ставка ЦБ → проверяй через WebSearch "ключевая ставка ЦБ РФ сегодня"
- Курс рубля (USDRUB) → проверяй через WebSearch
- Цена нефти Brent/Urals → проверяй через WebSearch
- Инфляция → проверяй через WebSearch
- Текущие котировки акций → проверяй через WebSearch "цена акции [тикер] MOEX"

**Цифры в knowledge/ файлах — это УЧЕБНЫЙ МАТЕРИАЛ (формулы, методы, пороги).**
**Актуальные макроданные — ВСЕГДА из интернета.**

Если WebSearch недоступен — предупреди пользователя: "Данные из базы знаний, могут быть неактуальны. Рекомендую перепроверить на smart-lab.ru"
